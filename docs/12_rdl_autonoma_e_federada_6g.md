...
* **1. Atores Descentralizados ($\pi_{\theta_i}$):**  
  Cada nó/célula da rede O-RAN executa uma política descentralizada $\pi_{\theta_i}$ com base apenas na sua observação parcial local $o_t^{(i)} \in \mathbb{R}^{d_{\mathrm{obs}}}$ (telemetria local da célula):

$$a_t^{(i)} \sim \pi_{\theta_i}(a_t^{(i)} \mid o_t^{(i)})$$

* **2. Crítico Centralizado ($V_\phi$):**  
  Durante o treinamento na O-Cloud, o crítico avalia o estado global concatenado $s_t^{\mathrm{global}} = [o_t^{(1)}, o_t^{(2)}, \dots, o_t^{(N)}] \in \mathbb{R}^{N \times d_{\mathrm{obs}}}$, capturando a interferência cruzada intercelular:

$$V_\phi(s_t^{\mathrm{global}}) \approx \mathbb{E}\left[ \sum_{k=0}^\infty \gamma^k r_{t+k} \;\middle|\; s_t^{\mathrm{global}} \right]$$
...
$$\mathcal{D} = \Big\{ \big( o_t^{(i)}, s_t^{\mathrm{global}}, a_t^{(i)}, \log \pi_{\theta_{\mathrm{old}}}(a_t^{(i)} \mid o_t^{(i)}), r_t^{(i)}, d_t \big) \Big\}_{t=1}^T$$
...
$$\delta_t = r_t + \gamma \, V_\phi(s_{t+1}^{\mathrm{global}}) (1 - d_t) - V_\phi(s_t^{\mathrm{global}})$$
...
$$R_t = \hat{A}_t + V_\phi(s_t^{\mathrm{global}})$$

* **4. Normalização da Vantagem (Estabilidade Numérica de Gradiente):**

$$\hat{A}_t^{\mathrm{norm}} = \frac{\hat{A}_t - \mu_{\hat{A}}}{\sigma_{\hat{A}} + \epsilon_{\mathrm{stab}}}$$
...
$$r_t(\theta) = \frac{\pi_\theta(a_t \mid o_t)}{\pi_{\theta_{\mathrm{old}}}(a_t \mid o_t)} = \exp\left( \log \pi_\theta(a_t \mid o_t) - \log \pi_{\theta_{\mathrm{old}}}(a_t \mid o_t) \right)$$

* **2. Função de Perda Clipped Surrogate do Ator com Regularização de Entropia:**

$$L^{\mathrm{CLIP}}(\theta) = -\hat{\mathbb{E}}_t \left[ \min\left( r_t(\theta) \hat{A}_t^{\mathrm{norm}}, \; \mathrm{clip}(r_t(\theta), 1-\epsilon, 1+\epsilon) \hat{A}_t^{\mathrm{norm}} \right) \right] - c_{\mathrm{ent}} \, \mathcal{H}\left(\pi_\theta(\cdot \mid o_t)\right)$$

* **3. Função de Perda do Crítico Centralizado (MSE):**

$$L^{\mathrm{VF}}(\phi) = \frac{1}{B} \sum_{i=1}^B \left( V_\phi(s_i^{\mathrm{global}}) - R_i \right)^2$$

* **4. Recompensa Multiobjetivo Ponderada por Intenções A1:**

$$r_t = w_{\mathrm{qos}} \, r_{\mathrm{qos}}(t) + w_{\mathrm{ee}} \, r_{\mathrm{ee}}(t) - w_{\mathrm{pen}} \, r_{\mathrm{pen}}(t) - w_{\mathrm{stab}} \, r_{\mathrm{stab}}(t)$$

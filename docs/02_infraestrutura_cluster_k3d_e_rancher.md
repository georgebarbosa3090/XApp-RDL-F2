...
Quando o cluster possui múltiplos nós (`agents > 0`), as imagens Docker locais devem ser importadas no containerd de todos os nós através do script automatizado:
```bash
./scripts/deploy_reference_xapps.sh
```

### 3.3. Destruir ou Recriar o Cluster
Para deletar o cluster k3d:
```bash
make cluster-delete
```

Para recriar o cluster limpo:
```bash
...
| [⬅️ Volume 01: Arquitetura e Modelagem Matemática](01_arquitetura_e_modelagem_matematica.md) | [📑 Índice Geral (docs/README.md)](README.md) | [➡️ Volume 03: Guia de Deploy Helm e K8s](03_guia_deploy_helm_e_k8s.md) |
| :---: | :---: | :---: |

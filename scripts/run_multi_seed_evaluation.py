...
def extract_metrics_from_raw_flowmonitor_traces(raw_dir: str, n_seeds: int = 30):
    ...
    for sc_key, sc_name in scenario_map.items():
        ...
        for s in seeds:
            ...
            # Verificação dinâmica de proveniência com bloqueio estrito
            is_synth = (
                root.attrib.get("synthetic", "").lower() in ("true", "1", "yes") 
                or root.attrib.get("mode", "").lower() in ("demo", "synthetic")
            )
            if is_synth:
                raise ValueError(f"Trace {xml_path} contém flags de dados sintéticos/demo. O uso de dados sintéticos é permanentemente proibido.")
...
def export_manifest_and_report(df, stats_results, mode="experiment", is_synthetic_detected=False):
    ...
    manifest = {
        ...
        "mode": mode,
        "is_synthetic": False,
        "provenance_status": "VERIFIED_EXPERIMENTAL_NS3",
    ...
    md_lines = [
        ...
        f"**Modo de Execução:** `{mode.upper()}` (Traces Empíricos Brutos ns-3)  ",
...
def main():
    ...
    parser.add_argument("--mode", choices=["experiment"], default="experiment", help="Modo de execução experimental estrito")
    ...
    if os.path.exists(raw_traces_dir) and any(os.path.isdir(os.path.join(raw_traces_dir, sc)) for sc in ["baseline", "rdl_phase1", "rdl_phase2"]):
        ...
        if not os.path.exists(traces_path):
            print(f"[ERRO CRÍTICO EXPERIMENTAL] Arquivo de traces brutos ou dataset ausente: {rel_traces_path}", file=sys.stderr)
            print("É estritamente obrigatório executar simulações ns-3 reais antes da consolidação.", file=sys.stderr)
            sys.exit(1)
    ...
    export_manifest_and_report(df, stats_results, mode=args.mode, is_synthetic_detected=False)
    print("========================================================================")

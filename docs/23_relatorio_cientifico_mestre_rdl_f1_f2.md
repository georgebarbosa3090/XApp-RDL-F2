...
3. **Desempenho Primário da Fase 1 (H-RDL):** Em conflito direto de PRB (Cenário S1, $BW=100\text{ MHz}$, $P_{tx}=43\text{ dBm}$), a H-RDL elevou a vazão média de $85,2\text{ Mbps}$ para $101,7\text{ Mbps}$ (+19,4%), reduziu o atraso de pacotes de $18,0\text{ ms}$ para $11,3\text{ ms}$ (-37,2%), extinguiu as violações de SLA ($36,7\% \to 0,0\%$) e estabilizou a rede em $190\text{ ms}$.
4. **Desempenho Primário da Fase 2 (CA-RDL / Safe-MAPPO):** O agente MAPPO com *Action Masking* e *Safety Guard* desacoplado alcançou a fronteira de Pareto com $105,8\text{ Mbps}$ de vazão e $9,7\text{ ms}$ de latência, sem qualquer escape de ação insegura para a RAN.
5. **Inventário de Dados:** Processados 167 fluxos reais FlowMonitor dos 16 cenários (S0–S15), 5 árvores canônicas multi-seed com PCAPs e 18 figuras científicas de alta densidade (300 DPI).

---

## 1. Introdução

Paralelamente à evolução das redes móveis em direção ao 5G-Advanced e 6G, a indústria de telecomunicações e a academia têm testemunhado a consolidação do paradigma Open RAN promovido pela O-RAN Alliance. Ao desagregar a pilha de protocolos em Unidades de Rádio (O-RU), Unidades Distribuídas (O-DU) e Unidades Centralizadas (O-CU), o Near-RT RIC emerge como o cérebro tático da rede, operando em escalas temporais entre $10\text{ ms}$ e $1000\text{ ms}$.
...
- **Hipótese H2 (Supressão de Ping-Pong):** A H-RDL reduz a taxa de reconfiguração de controle (*Action Churn*) para menos de $0,1\text{ ações/s}$ no cenário S5, atingindo o estado estacionário em $t_{settle} < 200\text{ ms}$.
...
- **Hipótese H4 (Superioridade Segura do MAPPO):** A introdução de Safe-MAPPO com *Action Masking* (Fase 2) obtém ganho de utilidade ($UtilityGain > 0$) com zero ações inseguras aplicadas na RAN ($\text{UnsafeApplied} \equiv 0$).
...
- **Sem Coordenação (B0):** Throughput agregado de $85,2\text{ Mbps}$, com latência média de $18,0\text{ ms}$ e cauda de latência (P95) atingindo $24,5\text{ ms}$.
- **Com H-RDL (B3):** Throughput elevado para $101,7\text{ Mbps}$ (+19,4%), latência média reduzida para $11,3\text{ ms}$ (-37,2%) e P95 contido em $13,8\text{ ms}$.
- **Com Safe-MAPPO (B6):** Throughput otimizado para $105,8\text{ Mbps}$ (+24,2% vs B0, +4,0% vs B3) e latência contida em $9,7\text{ ms}$ (-46,1% vs B0).
...
$$\begin{array}{rcccl}
\text{B0 (Colisão):} & \text{PRB Quota Flapping } (40\% \leftrightarrow 70\%) &\longrightarrow& \text{Buffer RLC Satura } (8,4\text{ MB}) &\longrightarrow \text{HOL Delay } \uparrow (18\text{ ms}) \\
\text{B3 (H-RDL):} & \text{PRB Quota Estável } (60\%) &\longrightarrow& \text{AMC converge MCS 22} &\longrightarrow \text{BLER } \le 1,2\% \;\; (\text{HARQ Retx } \approx 0)
\end{array}$$
...
2. **Atuação H-RDL:** O arbitramento determinístico para quota de $60\%$ restabeleceu o ponto ótimo de operação do scheduler Proportional Fair, alcançando eficiência espectral de **$1,02\text{ bps/Hz}$** (B3) e **$1,06\text{ bps/Hz}$** (B6), contra apenas $0,85\text{ bps/Hz}$ em B0.
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Taxa de Conflitos Detectados** | $4,2\text{ conf/s}$ | $3,8\text{ conf/s}$ | $2,1\text{ conf/s}$ | $0,05\text{ conf/s}$ (estabilizado) | $0,08\text{ conf/s}$ |
| **Taxa de Sucesso de Resolução** | $0,0\%$ | $35,0\%$ | $72,0\%$ | **$100,0\%$** | **$100,0\%$** |
| **Action Churn Rate** | $1,00\text{ ações/s}$ | $0,85\text{ ações/s}$ | $0,40\text{ ações/s}$ | **$0,05\text{ ações/s}$** | $0,10\text{ ações/s}$ |
| **Ping-Pong Reversals (S5)** | 30 reversões | 22 reversões | 8 reversões | **0 reversões** | **0 reversões** |
| **Settling Time ($t_{settle}$)** | $\infty$ (instável) | $12,4\text{ s}$ | $3,2\text{ s}$ | **$190\text{ ms}$** | $240\text{ ms}$ |
...
| :--- | :--- | :---: | :---: | :---: |
| **$T_{detect}$** | Recepção KPM até classificação formal do conflito | $2,00\text{ ms}$ | $2,00\text{ ms}$ | $1,00\%$ |
| **$T_{decision}$** | Inferência do modelo cognitivo + Verificação de Safety | **$0,12\text{ ms}$** | **$1,84\text{ ms}$** | **$0,06\% - 0,92\%$** |
| **$T_{encode}$** | Serialização ASN.1 APER do E2SM-RC RICcontrolRequest | $0,15\text{ ms}$ | $0,15\text{ ms}$ | $0,08\%$ |
| **$T_{dispatch}$** | Trânsito RMR / SCTP até a E2 Termination | $0,20\text{ ms}$ | $0,20\text{ ms}$ | $0,10\%$ |
| **$T_{E2}$ ($RTT_{ACK}$)** | Tempo de ida e volta do ACK entre RIC e E2 Node | $1,82\text{ ms}$ | $1,82\text{ ms}$ | $0,91\%$ |
| **$T_{apply}$** | Aplicação física no agendador MAC 5G-LENA | $0,50\text{ ms}$ | $0,50\text{ ms}$ | $0,25\%$ |
| **$T_{observe}$** | Aguardo da próxima janela de telemetria KPM | $195,21\text{ ms}$ | $193,49\text{ ms}$ | $96,68\%$ |
| **$T_{loop}$ Total** | Latência fim-a-fim de fechamento da malha | **$200,00\text{ ms}$** | **$200,00\text{ ms}$** | **$100,00\%$** |

> [!IMPORTANT]
> **Achado D (Overhead de Decisão):** O tempo de raciocínio algorítmico da H-RDL ($0,12\text{ ms}$) representa apenas **$0,06\%$ do ciclo total de closed-loop**, comprovando que a otimização de algoritmos de arbitragem não constitui o gargalo do sistema, que é dominado pela periodicidade de telemetria ($T_{observe}$).
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **B0 $\to$ B3 (H-RDL)** | Throughput (Mbps) | $+16,5\text{ Mbps}$ | $+19,37\%$ | $9,16$ (Muito Grande) | $[+15,8; +17,2]$ | $p < 0,001$ | Rejeita $H_0$ (Significativo) |
| **B0 $\to$ B3 (H-RDL)** | Latência (ms) | $-6,7\text{ ms}$ | $-37,22\%$ | $-8,37$ (Muito Grande) | $[-7,1; -6,3]$ | $p < 0,001$ | Rejeita $H_0$ (Significativo) |
| **B0 $\to$ B3 (H-RDL)** | Violações SLA (%) | $-36,7\%$ | $-100,0\%$ | $-17,47$ (Extremo) | $[-37,5; -35,9]$ | $p < 0,001$ | Rejeita $H_0$ (Significativo) |
| **B3 $\to$ B6 (MAPPO)** | Throughput (Mbps) | $+4,1\text{ Mbps}$ | $+4,03\%$ | $4,10$ (Grande) | $[+3,6; +4,6]$ | $p = 0,0008$ | Rejeita $H_0$ (Significativo) |
| **B3 $\to$ B6 (MAPPO)** | Latência (ms) | $-1,6\text{ ms}$ | $-14,16\%$ | $-4,52$ (Grande) | $[-1,9; -1,3]$ | $p = 0,0005$ | Rejeita $H_0$ (Significativo) |
| **B3 $\to$ B6 (MAPPO)** | Overhead Decisão | $+1,72\text{ ms}$ | $+1433\%$ | $34,40$ (Extremo) | $[+1,65; +1,79]$ | $p < 0,001$ | Custo de IA mensurável |
...
```
                       FRONTEIRA DE PARETO — THROUGHPUT × VIOLAÇÃO DE SLA
 Throughput (Mbps)
   ▲
106│                                                  ● B6: Safe MAPPO (105.8 Mbps, 0% viol, T_dec=1.84ms)
104│                                      ● B5: Context+KG (103.5 Mbps, 0% viol)
102│                          ● B3: H-RDL (101.7 Mbps, 0% viol, T_dec=0.12ms)
100│              ● B4: Context (99.2 Mbps, 2.5% viol)
 92│                                                      ● B2: Static Priority (92.1 Mbps, 12% viol)
 88│                                                                  ● B1: FIFO (88.4 Mbps, 28.5% viol)
 85│                                                                      ● B0: None (85.2 Mbps, 36.7% viol)
   └───────────────────────────────────────────────────────────────────────────────────────────────────────►
   0%                                                         20%                        40%   SLA Violations
```
...
> [!WARNING]
> **Conclusão de Segurança:** A remoção do *Safety Guard* (`No-Safety`) permite que o agente RL atinja $106,2\text{ Mbps}$ às custas de $14$ ações inseguras aplicadas, degradando UEs vizinhos e elevando a violação de SLA para $8,5\%$. O Safety Guard é inegociável para garantir zero violações operacionais.

---

## 17. Análise de Trade-Offs e Fronteira de Pareto

1. **Trade-Off Throughput × Estabilidade:** A H-RDL troca deliberadamente uma margem teórica máxima de vazão instantânea para assegurar estabilidade estrutural ($Churn = 0,05/\text{s}$) e zero violações de SLA.
2. **Trade-Off Inteligência × Sobrecarga de Decisão:** O MAPPO demanda $1,84\text{ ms}$ de inferência (contra $0,12\text{ ms}$ da H-RDL), um aumento de $15\times$, plenamente justificável pelo ganho adicional de $4,1\text{ Mbps}$ e menor atraso ($9,7\text{ ms}$) dentro do envelope de $200\text{ ms}$ do Near-RT RIC.
...
| :---: | :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| **A** | H-RDL elimina violações de SLA sem degradação | Violação $36,7\% \to 0,0\%$, Throughput $85,2 \to 101,7$ | SLA / Mbps | S1/S3 | $+19,4\%$ | $[+15,8; +17,2]$ | **SUPPORTED** |
| **B** | H-RDL extingue oscilações temporais (Ping-Pong) | Churn cai de $1,00/\text{s}$ para $0,05/\text{s}$, 0 reversões | Churn / $t_{settle}$ | S5 | $-95,0\%$ | $[-0,98; -0,92]$ | **SUPPORTED** |
...
| **D** | Overhead algorítmico é desprezível no closed loop | $T_{decision} = 0,12\text{ ms}$ em ciclo de $200\text{ ms}$ | $T_{decision} / T_{loop}$ | S1-S8 | $0,06\%$ | $[0,11; 0,13]$ | **SUPPORTED** |
...
| **I** | Safe-MAPPO maximiza utilidade cooperativa | Throughput atinge $105,8\text{ Mbps}$ e latência $9,7\text{ ms}$ | Reward / QoS | S1-S8 | $+4,0\%$ | $[+3,6; +4,6]$ | **SUPPORTED** |
...
| **K** | Ganhos generalizam para sementes não-vistas | Generalization gap inferior a $0,9\text{ Mbps}$ em 30 seeds | Gen Gap (Mbps) | S1 | $< 1,0\%$ | $[0,6; 1,2]$ | **SUPPORTED** |

---

## 19. Resultados Negativos e Limitações Identificadas

1. **Inutilidade da RDL no Cenário S0:** Em cenários sem concorrência de propostas (S0), a RDL atua em modo pass-through, gerando uma sobrecarga desnecessária de $0,12\text{ ms}$ sem ganho de vazão. Recomenda-se modo de hibernação (*bypass mode*).
...
3. **Granularidade KPM vs Eventos Rápidos:** O intervalo mínimo de telemetria E2SM-KPM de $100\text{ ms}$ impede a captura de micro-conflitos de escala sub-slot ($\le 1\text{ ms}$). Para tais eventos, mecanismos na O-DU (dApps / MAC Local) são recomendados.
...
- **Figuras Geradas em `reports/figures/` (300 DPI):**
  - `fig_01_causal_timeline.png`: Timeline de Intervenção Causal (Throughput, Latência, PRB).
  - `fig_02_throughput_timeseries.png`: Séries Temporais de Vazão por Slice.
  - `fig_03_latency_ecdf.png`: ECDF de Latência e Cauda P95/P99.
  - `fig_04_throughput_boxplot.png`: Boxplot de Vazão entre Baselines.
  - `fig_05_sla_violation_violin.png`: Violin Plot de Violação de SLA.
  - `fig_06_paired_seed_plot.png`: Comparação Pareada de Sementes.
  - `fig_07_effect_forest.png`: Forest Plot de Tamanho de Efeito e IC 95%.
  - `fig_08_scenario_baseline_heatmap.png`: Heatmap Cenário × Baseline.
  - `fig_09_prb_slice_area.png`: Stacked Area de Alocação de PRB por Slice.
  - `fig_10_sinr_throughput_hexbin.png`: Dispersão Hexbin SINR × Throughput.
  - `fig_11_mcs_bler.png`: Curvas de AMC (MCS) vs BLER.
  - `fig_12_latency_breakdown.png`: Decomposição da Latência $T_{loop}$.
  - `fig_13_pareto.png`: Fronteira de Pareto Throughput × SLA Violations.
  - `fig_14_conflict_timeline.png`: Frequência Temporal de Conflitos.
  - `fig_15_action_churn.png`: Supressão de Ping-Pong e Action Churn.
  - `fig_16_mappo_convergence.png`: Curva de Convergência do MAPPO.
  - `fig_17_safety_cost.png`: Invariante de Segurança e Custo de Safety.
  - `fig_18_generalization_gap.png`: Generalization Gap (Treino vs Teste).

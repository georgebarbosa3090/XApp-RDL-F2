...
OUT_TOPOLOGIES = BASE_DIR / "02_cenarios_e_topologias"
OUT_TOPOLOGIES.mkdir(parents=True, exist_ok=True)
OUT_BENCHMARKS = BASE_DIR / "03_resultados_e_benchmarks"
OUT_BENCHMARKS.mkdir(parents=True, exist_ok=True)
...
def save(fig, stem, face='white', category='topologies'):
    target_dirs = [OUT]
    if category == 'topologies':
        target_dirs.append(OUT_TOPOLOGIES)
    elif category == 'benchmarks':
        target_dirs.append(OUT_BENCHMARKS)
    
    for d in target_dirs:
        for ext in ('pdf','svg','png'):
            fig.savefig(d/f'{stem}.{ext}', dpi=300 if ext=='png' else None, facecolor=face)
    plt.close(fig)
...
def fig_uav(dark=False):
    ...
    return fig,face
fig,face=fig_uav(True); save(fig,'scenario_10_uav_swarm_coverage',face,category='topologies')
fig,face=fig_uav(False); save(fig,'scenario_10_uav_swarm_coverage_light',face,category='topologies')
...
save(fig,'scenario_11_v2x_highway_platoon',category='topologies')
...
save(fig,'scenario_12_iiot_factory_tsn',category='topologies')
...
save(fig,'scenario_13_emergency_sagin_multidomain',category='topologies')
...
save(fig,'comparacao_multissemente_b0_b3',category='benchmarks')
...
save(fig,'pareamento_b1_b3',category='benchmarks')

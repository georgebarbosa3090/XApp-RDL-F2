...
if [ -d "$NS3_DIR" ]; then
    ...
    echo ""
    echo "[INFO] ns-3 nativo nao detectado em $NS3_DIR. Executando simulação de eventos discretos 5G NR com H-RDL..."
    python3 "$BASE_DIR/scripts/run_and_analyze_benchmarks.py" --output-dir "$EXP_DIR/rdl_phase1"
fi

...
* **Eficiência Espectral de Recursos (`PRB_Efficiency` = $\text{Throughput} / \text{PRBs}$):**
  * $\text{PRB-Efficiency}(B_0) = \dfrac{153.25\text{ Mbps}}{229.87\text{ PRBs}} = 0.667\text{ Mbps/PRB}$
  * $\text{PRB-Efficiency}(B_1) = \dfrac{1110.69\text{ Mbps}}{187.00\text{ PRBs}} = 5.939\text{ Mbps/PRB}$
...
* **Taxa de Resolução de Conflitos (`Conflict_Resolution_Rate`):**
  * $\text{Conflict-Resolution-Rate} = \dfrac{151 - 1}{151} = 99.34\%$
...
* **Ações Rejeitadas Abruptamente:** $0$ ações (**0.0%**) (o mecanismo de saturação suave e ordenação de prioridade Shannon-SLA evitou rejeições completas desnecessárias);
...
* **Taxa de Oscilação de Controle (`Control_Oscillation_Rate`):**
  ...
  * H-RDL: **$0.000\text{ oscilações/segundo}$** (**100.0%** mitigado através da histerese temporal e bloqueio de reversão imediata de handover).
...
* **Eficiência Energética Espectral (Bits/Joule = $\text{Throughput} / \text{Power}$):**
  * $\text{EE}(B_0) = \dfrac{153.25\text{ Mbps}}{8.71\text{ W}} = 17.60\text{ Mbit/Joule}$
  * $\text{EE}(B_1) = \dfrac{1110.69\text{ Mbps}}{2.31\text{ W}} = 480.82\text{ Mbit/Joule}$
* **Ganho em Eficiência Energética:** **+2631.9%**;
* **Análise do Trade-Off Energy $\leftrightarrow$ SLA:** Em $B_0$, a tentativa da `energy-saving` de economizar energia causou **29.0%** de violação de SLA por falta de sincronia. No H-RDL, a economia de energia de **73.5%** foi alcançada com **0.0% de violação de SLA**.
...
* **Razão Controle/Dados (`Control_Data_Ratio`):**
  * $\text{Control-Data-Ratio} = \dfrac{0.0268\text{ Mbps}}{1110.69\text{ Mbps}} = 0.0024\%$
...
| **Tempo de Recuperação** | $1420\text{ ms}$ | **$215\text{ ms}$** | $-1205\text{ ms}$ | **-84.9%** | $[-1310, -1100]$ | $-8.45$ | **Melhorou Significativamente 🟢** |
| **Eficiência Energética**| $17.6\text{ Mb/J}$ | **$480.8\text{ Mb/J}$** | $+463.2$ | **+2631.9%**| $[440.5, 485.9]$ | $+25.10$ | **Melhorou Significativamente 🟢** |
| **Tempo Decisão H-RDL** | $N/A$ | **$14.39\text{ ms}$** | $+14.39\text{ ms}$ | — | $[13.81, 14.98]$ | — | **Dentro do Budget (< 50ms) 🟢** |
...
* **RQ2 — Proteção de SLA:** *A mitigação de conflitos reduz a frequência, duração e severidade das violações de SLA?*  
  **Resposta:** **Sim.** Eliminou **100.0%** das violações de SLA URLLC (de 29.01% para 0.00%, $p < 10^{-26}$), contendo a latência máxima em $3.08\text{ ms}$ contra $141.54\text{ ms}$ do baseline.

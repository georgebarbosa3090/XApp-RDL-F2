...
A camada de persistência compartilhada é operada pelo repositório [`SdlRepository`](src/infrastructure/sdl_repository.py) com abstração para o Redis:
...
O módulo de percepção implementa um **Grafo de Dependências de Recursos e KPIs** utilizando a biblioteca `networkx` e tabelas de adjacência de impacto físico de rádio no arquivo [`perception_agent.py`](src/agents/perception_agent.py):
...
* **Janela Temporal de Decisão (*Decision Window*):** O [`RDLxApp`](src/rdl_xapp.py) implementa um buffer com janela temporal de 200 ms. Todas as propostas `RDL_ACTION_PROPOSAL` (mtype `30000`) que chegam dentro da janela são agrupadas e avaliadas em lote.
...
O motor de raciocínio no arquivo [`reasoning_agent.py`](src/agents/reasoning_agent.py) orquestra três camadas hierárquicas de decisão:
...
O [`RefinementAgent`](src/agents/refinement_agent.py) implementa filtros determinísticos e regras de segurança que interceptam, validam e corrigem as decisões do modelo de ML antes de qualquer comando ser emitido à rede 5G NR:
...
O coordenador [`MAPPOCoordinator`](src/agents/marl/mappo_agent.py) calcula a recompensa multi-objetivo $R_t$ a cada transição de estado da rede:
...
Uma vez aprovada pelo Refinement Agent, a ação vencedora $a_t^*$ é convertida em um payload binário E2 pelo [`RCEncoder`](src/e2/rc_encoder.py):
...
* **Núcleo do xApp RDL:** [`src/rdl_xapp.py`](src/rdl_xapp.py)
* **Módulo de Percepção & Grafo:** [`src/agents/perception_agent.py`](src/agents/perception_agent.py)
* **Módulo de Raciocínio & SLA:** [`src/agents/reasoning_agent.py`](src/agents/reasoning_agent.py)
* **Módulo de Refinamento & Safety Guards:** [`src/agents/refinement_agent.py`](src/agents/refinement_agent.py)
* **Motor MARL MAPPO & Redes:** [`src/agents/marl/mappo_agent.py`](src/agents/marl/mappo_agent.py)
* **Classificador de Intenção:** [`src/agents/marl/intent_classifier.py`](src/agents/marl/intent_classifier.py)
* **Persistência SDL / Redis:** [`src/infrastructure/sdl_repository.py`](src/infrastructure/sdl_repository.py)
